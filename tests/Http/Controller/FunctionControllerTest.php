...
describe('FunctionController', function (): void {
    ...
    describe('Sad Paths', function (): void {
        test('returns HTTP 200 with error in response body for non-existent function', function (): void {
            // Arrange & Act & Assert
            // Per Forrst spec: HTTP status MUST be 200 for ALL Forrst responses.
            // Errors are communicated via the response body's `errors` field.
            FunctionCaller::call('rpc-call-of-non-existent-method');
        });

        test('returns HTTP 200 with parse error for invalid JSON', function (): void {
            FunctionCaller::call('rpc-call-with-invalid-json');
        });

        test('returns HTTP 200 with invalid request error for malformed request object', function (): void {
            FunctionCaller::call('rpc-call-with-invalid-request-object');
        });

        test('returns HTTP 200 with parse error for invalid batch JSON', function (): void {
            FunctionCaller::call('rpc-call-batch-invalid-json');
        });

        test('returns HTTP 200 with invalid request error for empty array batch', function (): void {
            FunctionCaller::call('rpc-call-with-an-empty-array');
        });

        test('returns HTTP 200 with errors for invalid batch items', function (): void {
            FunctionCaller::call('rpc-call-with-an-invalid-batch-but-not-empty');
        });

        test('returns HTTP 200 with invalid request error for completely invalid batch', function (): void {
            FunctionCaller::call('rpc-call-with-invalid-batch');
        });
    });
